...
const Sidebar = () => {
    ...
    return (
        <div ref={sidebarRef} className={`max-h-full bg-gray-900 text-white space-y-6 py-7 absolute inset-y-0 left-0 transform transition-all duration-500 ease-in-out overflow-x-hidden h-screen z-50 ${isOpen ? 'translate-x-0 w-64' : 'translate-x-0 w-16'}`}>
            ...
            <nav>
                <Link to="/admin" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700 flex items-center">
                    ...
                    <span className={`ml-4 ${isOpen ? 'inline' : 'hidden'}`}>UserManagement</span>
                </Link>
                ...
                <Link to="/settings" className="block py-2.5 px-4 rounded transition duration-200 hover:bg-gray-700 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="h-6 w-6">
                    ...
                    <span className={`ml-4 ${isOpen ? 'inline' : 'hidden'}`}>Settings</span>
                </Link>
            </nav>
        </div>
    );
